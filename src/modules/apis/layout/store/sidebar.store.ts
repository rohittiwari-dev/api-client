...
import { RequestType } from "@/modules/apis/requests/types/store.types";
...
const updateInTree = (
  ...
  return items.map((item) => {
    ...
    if (item.type === "COLLECTION") {
      return {
        ...item,
        children: updateInTree(item.children || [], id, updates),
      };
    }
    return item;
  });
    ...
    persist(
      (set) => ({
        ...
        addItem: (item, parentId) =>
          set((state) => ({
            items: addToTree(state.items, item, parentId ?? null),
          })),
        moveItem: (itemId, newParentId) =>
          set((state) => {
            const [itemsWithoutMoved, movedItem] = findAndRemove(
              state.items,
              itemId
            );
            if (!movedItem) return state;
            return {
              items: addToTree(itemsWithoutMoved, movedItem, newParentId),
            };
          }),
        reorderItems: (orderedIds, parentId, type) =>
          set((state) => {
            const reorderAtLevel = (
              items: SidebarItemInterface[]
            ): SidebarItemInterface[] => {
              if (!parentId) {
              ...
              return items.map((item) => {
                ...
                if (item.type === "COLLECTION") {
                  return {
                    ...item,
                    children: reorderAtLevel(item.children || []),
                  };
                }
                return item;
              });
