...
import useWebsocketStore, { type WebSocketConnectionOptions } from '../../store/websocket.store';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const directionColors: Record<string, { bg: string; text: string; border: string }> = {
    sent: {
...
export const WebSocketMessageLog: React.FC<WebSocketMessageLogProps> = ({
    ...
    const connectionOptions = useWebsocketStore((state) => state.connectionOptions[requestId]);
    ...
    const handleCopyAll = async () => {
        const allContent = filteredMessages
            .map(msg => `[${new Date(msg.timestamp).toLocaleTimeString()}] ${msg.direction.toUpperCase()}: ${msg.content}`)
            .join('\n\n');
        try {
        ...
        <div className={cn('flex flex-col h-full min-h-0 overflow-hidden', className)}>
            ...
            <div className="flex items-center gap-2 p-3 border-b border-violet-500/15 glass-subtle">
                <div className="flex items-center gap-2 flex-1">
                    ...
                    <Badge variant="secondary" className="text-[10px] px-2 py-0.5 h-5 rounded-full bg-gradient-to-r from-violet-500/20 to-indigo-500/20 text-violet-600 dark:text-violet-400 border border-violet-500/30 font-bold">
                    ...
                    <DropdownMenuTrigger asChild>
                        ...
                        >
                            ...
                            <span className="font-medium">
                                {filterDirection === 'all' ? 'All' : <span className="capitalize">{filterDirection}</span>}
                            </span>
                    ...
                    <DropdownMenuContent align="end" className="w-32 p-1">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-muted-foreground tabular-nums">{messageCounts.all}</span>
                                {filterDirection === 'all' && <IconCheck className="size-3 text-primary" />}
                            </div>
                        </DropdownMenuItem>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-muted-foreground tabular-nums">{messageCounts.sent}</span>
                                {filterDirection === 'sent' && <IconCheck className="size-3 text-primary" />}
                            </div>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-muted-foreground tabular-nums">{messageCounts.received}</span>
                                {filterDirection === 'received' && <IconCheck className="size-3 text-primary" />}
                            </div>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-muted-foreground tabular-nums">{messageCounts.system}</span>
                                {filterDirection === 'system' && <IconCheck className="size-3 text-primary" />}
                            </div>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                <span className="text-[9px] text-muted-foreground tabular-nums">{messageCounts.error}</span>
                                {filterDirection === 'error' && <IconCheck className="size-3 text-primary" />}
                            </div>
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
                ...
                <TooltipProvider>
                    <Tooltip>
                        ...
                        <TooltipContent side="bottom" className="text-[10px] rounded-md py-1 px-2">
                            Download as JSON
                        </TooltipContent>
                    </Tooltip>
                </TooltipProvider>
                        ...
                        <TooltipTrigger asChild>
                            <Button
                                ...
                                className={cn(
                                    ...
                                    autoScroll && "bg-gradient-to-r from-violet-500/20 to-indigo-500/20 text-violet-600 dark:text-violet-400 hover:from-violet-500/30 hover:to-indigo-500/30 border border-violet-500/30"
                                )}
                            >
                ...
                <div className="p-3 space-y-2">
                    {filteredMessages.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-3">
                            {messages.length === 0 ? (
                                <>
                                    <div className="size-16 rounded-lg bg-gradient-to-br from-violet-500/20 to-indigo-500/10 border border-violet-500/20 flex items-center justify-center shadow-lg shadow-violet-500/10">
                                        <IconInfoCircle className="size-8 text-violet-500/40" />
                                    </div>
                                    <div className="text-center">
                                        <span className="text-sm font-bold block text-foreground/80">No messages yet</span>
                                        <span className="text-[11px] text-muted-foreground/60 mt-1 block">Messages will appear here when sent or received</span>
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <span className="text-sm font-bold text-foreground/80">No messages match your filter</span>
                                </>
                            )}
                        </div>
                    ) : (
                                    ...
                                    <div className={cn(
                                        ...
                                        <div className="flex items-center gap-2 px-2.5 py-1 bg-muted/30">
                                            ...
                                            <div className={cn(
                                                ...
                                                {msg.direction === 'sent' && <IconArrowUp className="size-3" />}
                                                {msg.direction === 'received' && <IconArrowDown className="size-3" />}
                                                {msg.direction === 'system' && <IconInfoCircle className="size-3" />}
                                                {msg.direction === 'error' && <IconX className="size-3" />}
                                        ...
                                        <div className="px-2.5 py-2">
                                            <pre className="text-[11px] font-mono whitespace-pre-wrap break-words text-foreground/90 m-0 leading-relaxed">
                                                {msg.content}
                                            </pre>
                                        </div>
                                    </div>
                    ...
                    <DialogHeader className="p-4 border-b border-border/40 bg-muted/40">
                        <DialogTitle className="text-sm font-medium flex items-center gap-2">
                            <div className={cn(
                                ...
                                detailsMessage?.direction === 'sent' && 'bg-emerald-500/10 text-emerald-500',
                                detailsMessage?.direction === 'received' && 'bg-blue-500/10 text-blue-500',
                                detailsMessage?.direction === 'system' && 'bg-amber-500/10 text-amber-500',
                                detailsMessage?.direction === 'error' && 'bg-rose-500/10 text-rose-500'
                            )}>
                                {detailsMessage?.direction === 'sent' && <IconArrowUp className="size-4" />}
                                {detailsMessage?.direction === 'received' && <IconArrowDown className="size-4" />}
                                {detailsMessage?.direction === 'system' && <IconInfoCircle className="size-4" />}
                                {detailsMessage?.direction === 'error' && <IconX className="size-4" />}
                            </div>
                            Message Details
                        </DialogTitle>
                        <DialogDescription className="text-xs text-muted-foreground ml-9">
                            {detailsMessage && new Date(detailsMessage.timestamp).toLocaleString()}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="rounded-lg border border-border/50 bg-muted/30 overflow-hidden">
                                    <div className="px-3 py-2 border-b border-border/30">
                                        <span className="text-[10px] text-muted-foreground uppercase font-medium">URL</span>
                                        <p className="text-xs font-mono text-foreground mt-0.5 break-all">{connectionOptions.url}</p>
                                    </div>
                                    {connectionOptions.parameters && connectionOptions.parameters.filter(p => p.isActive !== false && p.key).length > 0 && (
                                        <div className="px-3 py-2 border-b border-border/30">
                                            <span className="text-[10px] text-muted-foreground uppercase font-medium">Parameters</span>
                                            <div className="mt-1 space-y-1">
                                                {connectionOptions.parameters.filter(p => p.isActive !== false && p.key).map((p, i) => (
                                                    <div key={i} className="flex items-center gap-2 text-xs font-mono">
                                                        <span className="text-primary">{p.key}</span>
                                                        <span className="text-muted-foreground">=</span>
                                                        <span className="text-foreground/80">{p.value || '(empty)'}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                    {connectionOptions.headers && connectionOptions.headers.filter(h => h.isActive !== false && h.key).length > 0 && (
                                        <div className="px-3 py-2">
                                            <span className="text-[10px] text-muted-foreground uppercase font-medium">Headers</span>
                                            <div className="mt-1 space-y-1">
                                                {connectionOptions.headers.filter(h => h.isActive !== false && h.key).map((h, i) => (
                                                    <div key={i} className="flex items-center gap-2 text-xs font-mono">
                                                        <span className="text-primary">{h.key}</span>
                                                        <span className="text-muted-foreground">:</span>
                                                        <span className="text-foreground/80">{h.value || '(empty)'}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center gap-2 text-xs font-medium text-foreground">
                                ...
                                {detailsMessage?.format === 'json' && (
                                    <Badge variant="secondary" className="ml-auto text-[9px] px-1.5 py-0 h-4 bg-primary/10 text-primary">
                                        JSON
                                    </Badge>
                                )}
                            </div>
                            <div className="rounded-lg border border-border/50 bg-muted/30 p-3">
                                <pre className="text-xs font-mono whitespace-pre-wrap break-words text-foreground/90 m-0 leading-relaxed max-h-[200px] overflow-y-auto">
                        ...
                        <div className="flex items-center gap-4 pt-2 border-t border-border/30 text-[10px] text-muted-foreground">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span className="font-mono">{detailsMessage?.id.slice(0, 8)}...</span>
                            </div>
                        </div>
                    </div>
                </DialogContent>
