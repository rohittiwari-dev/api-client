...
import useSidebarStore, { SidebarItemInterface, SidebarCollectionItemInterface } from "../../store/sidebar.store";
...
export function DraggableSidebarTree({
    ...
    const flatItems = useMemo(() => {
        const flat: Map<string, { item: SidebarItemInterface; parentId: string | null }> = new Map();
    ...
    const isDescendant = (parentId: string, childId: string): boolean => {
        ...
        for (const child of children) {
            ...
            if (child.type === "COLLECTION" && isDescendant(child.id, childId)) return true;
        }
        return false;
    };
    ...
    function handleDragMove(event: DragMoveEvent) {
        ...
        if (isActiveRequest && isCrossParent && overData.item.type !== "COLLECTION" && overData.parentId) {
        ...
        if (isActiveRequest && isCrossParent && overData.item.type === "COLLECTION") {
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        if (!finalOverId || (finalOverId !== "ROOT" && active.id === finalOverId)) return;
        ...
        if (currentDropPosition === "inside") {
            ...
            if (activeItem.type === "COLLECTION") {
                if (overItem.id === activeItem.id || isDescendant(activeItem.id, overItem.id)) {
                    return;
                }
            }
            ...
            const getSiblings = (parentId: string | null, type: "COLLECTION" | "REQUEST") => {
                if (parentId) {
                    const parent = flatItems.get(parentId)?.item as SidebarCollectionItemInterface;
                    return (parent?.children || []).filter(
                        type === "COLLECTION" ? (c) => c.type === "COLLECTION" : (c) => c.type !== "COLLECTION"
                    );
                }
                return items.filter(
                    type === "COLLECTION" ? (i) => i.type === "COLLECTION" : (i) => i.type !== "COLLECTION"
                );
            };

            if (activeItem.type === "COLLECTION") {
                if (overItem.type !== "COLLECTION" && activeParentId === overParentId) return;
                ...
                if (activeParentId !== targetParentId) {
                    ...
                    if (oldIndex !== -1 && newIndex !== -1 && oldIndex !== newIndex) {
                        ...
                        reorderCollectionsMutation.mutate({ orderedIds, parentId: targetParentId });
                    }
                }
            } else {
                if (overItem.type === "COLLECTION" && currentDropPosition === "before") return;
                ...
                if (activeItem.collectionId !== newCollectionId) {
                    ...
                    if (oldIndex !== -1 && newIndex !== -1 && oldIndex !== newIndex) {
                        orderedIds.splice(oldIndex, 1);
                        if (currentDropPosition === "after" && overItem.type !== "COLLECTION") {
                            orderedIds.splice(newIndex, 0, activeItem.id);
                        } else {
                            orderedIds.splice(Math.max(0, newIndex), 0, activeItem.id);
                        }
                        reorderRequestsMutation.mutate({ orderedIds, parentId: newCollectionId });
                    }
                }
            }
        }
    ...
    return (
        ...
        >
            <SidebarTreeContext.Provider value={{
                activeId,
                overId,
                dropPosition,
                justMovedId,
                ...
                <div
                    ...
                    className={cn(
                        ...
                        isRootHighlight ? "bg-primary/5 border-2 border-dashed border-primary/30" : "border-2 border-transparent"
                    )}
                >
                ...
                <DragOverlay dropAnimation={null}>
                    {activeItem && (
                        <div className="bg-background border border-primary/20 rounded-lg shadow-lg px-3 py-2 text-sm opacity-90 backdrop-blur-sm">
                            <span className="font-medium mr-2">{activeItem.type === "COLLECTION" ? "📁" : "📄"}</span>
                            {activeItem.name}
                        </div>
                    )}
                </DragOverlay>
