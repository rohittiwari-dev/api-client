...
const CopyButton = ({ value, size = 'sm' }: { value: string; size?: 'sm' | 'md' }) => {
    ...
    return (
        ...
        >
            {copied ? <Check className={size === 'sm' ? "size-3" : "size-3.5"} /> : <Copy className={size === 'sm' ? "size-3" : "size-3.5"} />}
        </button>
    );
};
...
const RequestInfoPanel = () => {
    ...
    if (!activeRequest) {
        return (
            <div className="flex flex-col items-center justify-center h-full py-16 px-8">
                ...
                <p className="text-sm font-medium text-foreground mb-1">No Request Selected</p>
    ...
    const activeParams = activeRequest.parameters?.filter(p => p.isActive) || [];
    const authType = activeRequest.auth?.type || 'NONE';
    const resolvedUrl = activeRequest.url ? getResolvedUrl(activeRequest.url) : '';
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="rounded-xl border border-border/40 overflow-hidden">
                            {activeHeaders.slice(0, 4).map((h, i) => (
                                <div key={i} className="flex items-center gap-2 px-3 py-2 text-[11px] border-b border-border/20 last:border-0 hover:bg-muted/30">
                                    <span className="font-mono font-medium text-foreground shrink-0">{h.key}</span>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="rounded-xl border border-border/40 overflow-hidden">
                            {activeParams.slice(0, 4).map((p, i) => (
                                <div key={i} className="flex items-center gap-2 px-3 py-2 text-[11px] border-b border-border/20 last:border-0 hover:bg-muted/30">
                                    <span className="font-mono font-medium text-foreground shrink-0">{p.key}</span>
                                    <span className="text-muted-foreground/50">=</span>
                                    <span className="font-mono text-muted-foreground truncate">{p.value}</span>
                                </div>
                            ))}
