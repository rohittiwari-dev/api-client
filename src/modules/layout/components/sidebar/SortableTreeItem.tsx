...
export function SortableTreeItem({ item, overId, activeId, dropPosition, collapseKey = 0, justMovedId }: SortableTreeItemProps) {
    const {
        attributes,
        listeners,
        setNodeRef,
...
function RequestTreeItem({
    ...
    const { activeTab, openRequest, requests } = useTabRequestSync();
    ...
    const methodBg = methodBgMap[method] || "bg-muted text-muted-foreground border-border";
        ...
        >
            ...
            <SidebarMenuButton
                ...
                onClick={() => {
                    openRequest(
                        {
                            title: currentRequest.name || "New Request",
                            type: (currentRequest.type as RequestType | "NEW") || "NEW",
                            id: item.id,
                ...
                <div className="flex items-center w-full gap-2">
                    ...
                    {currentRequest.type === "API" ? (
                        ...
                        <span className="shrink-0 p-1 rounded-md bg-muted/50 border border-border/50">
                            <RequestIcon type={(currentRequest.type as RequestType) || "API"} className="size-3" />
                        </span>
                    )}
                    ...
                    <div className="flex flex-col min-w-0 flex-1 gap-0.5">
                        <div className="flex items-center gap-1.5">
                            ...
                            {isUnsaved && (
                                <span className="shrink-0 size-1.5 rounded-full bg-blue-500" title="Unsaved" />
                            )}
                        </div>
                        {displayUrl && (
...
function CollectionTreeItem({
    ...
    const { activeTab } = useTabRequestSync();
    const [isOpen, setIsOpen] = React.useState(item.id === activeTab?.collectionId);
    ...
    return (
        <motion.li
            ...
            className={cn(
                ...
                isDragging && "opacity-40 bg-muted/50 rounded-lg",
                isOver && dropPosition === "inside" && "ring-2 ring-primary rounded-lg bg-primary/10",
                isJustMoved && "ring-2 ring-primary/50"
            )}
        >
            {/* Drop indicator - before */}
