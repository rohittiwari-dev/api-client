...
import { CommandIcon, SearchIcon, Code2, FolderPlus, Wifi, Radio } from 'lucide-react';
...
const SearchPanel = () => {
	...
	const { openRequest } = useTabRequestSync();
	...
	const handleCreateRequest = useCallback((type: 'API' | 'WEBSOCKET' | 'SOCKET_IO') => {
		if (!activeWorkspace?.id) return;
		...
		openRequest(tabData, requestData);
		setOpen(false);
		setQuery('');
	...
	const handleOpenRequest = useCallback((request: typeof requests[0]) => {
		const tabData = {
			id: request.id,
			title: request.name,
			type: request.type as 'API' | 'WEBSOCKET' | 'SOCKET_IO',
			method: request.method as HttpMethod | undefined,
			unsaved: request.unsaved || false,
			workspaceId: request.workspaceId,
			collectionId: request.collectionId,
		};

		// The request is already in the store, just set it active
		openRequest(tabData, request);
		setOpen(false);
		setQuery('');
	}, [openRequest]);
	...
	const getTypeIcon = (type: string, method: HttpMethod | null | undefined) => {
		...
		return (
			<span className={cn("text-[10px] font-bold", getMethodBadgeColor(method))}>
				{method || 'GET'}
			</span>
		);
	};
					...
					<CommandGroup heading="Create New">
						...
						>
							...
							<div className="flex flex-col gap-0.5 flex-1">
								...
								<span className="text-xs text-muted-foreground">Create a REST API request</span>
							</div>
						...
						>
							...
							<div className="flex flex-col gap-0.5 flex-1">
								...
								<span className="text-xs text-muted-foreground">Create a WebSocket connection</span>
							</div>
						</CommandItem>
						...
						>
							...
							<div className="flex flex-col gap-0.5 flex-1">
								...
								<span className="text-xs text-muted-foreground">Create a Socket.IO connection</span>
							</div>
						</CommandItem>
						...
						>
							...
							<div className="flex flex-col gap-0.5 flex-1">
								...
								<span className="text-xs text-muted-foreground">Organize your requests</span>
							</div>
							<CommandShortcut>⌘⇧N</CommandShortcut>
						</CommandItem>
					</CommandGroup>
								...
								>
									{request.type === "API" ? (
										...
										<div className={cn(
											...
											request.type === 'WEBSOCKET' ? "bg-violet-500/10" :
												request.type === 'SOCKET_IO' ? "bg-indigo-500/10" : "bg-muted"
										)}>
