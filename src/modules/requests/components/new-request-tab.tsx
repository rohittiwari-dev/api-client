...
import { useTabRequestSync } from '../hooks/use-tab-request-sync';
...
const NewRequestTabContent = () => {
	const { openRequest, activeTab } = useTabRequestSync();
		...
		<div className="flex flex-col items-center justify-center h-full w-full px-8 py-12 bg-gradient-to-br from-background via-background to-muted/20">
			...
			<div className="mt-10 text-center">
				<p className="text-xs text-muted-foreground/50">
					Press <kbd className="px-1.5 py-0.5 rounded bg-muted border border-border/50 text-[10px] font-mono">Ctrl</kbd> + <kbd className="px-1.5 py-0.5 rounded bg-muted border border-border/50 text-[10px] font-mono">N</kbd> for quick access
				</p>
			</div>
		</div>
	);
