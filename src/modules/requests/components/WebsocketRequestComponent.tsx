...
import useWebsocketStore, { type MessageFormat } from '../store/websocket.store';
...
import WebSocketMessageComposer, { type MessageFormat as ComposerFormat } from './shared/WebSocketMessageComposer';
import WebSocketSavedMessages, { type SavedMessage } from './shared/WebSocketSavedMessages';
...
const WebsocketRequestComponent = () => {
	...
	const handleSaveMessage = (name: string, content: string, format: ComposerFormat) => {
			...
			<div className="flex w-full items-center !border-t-0 gap-3 px-4 py-3 border-b border-violet-500/15 glass-subtle">
				...
				{status === 'connecting' && (
					<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-500/15 to-orange-500/15 border border-amber-500/30 shadow-md shadow-amber-500/10">
						...
						<span className="text-[11px] font-bold text-amber-600 dark:text-amber-400">Connecting</span>
					</div>
				)}
					...
					<ResizablePanelGroup direction="horizontal" className="h-full w-full">
						<ResizablePanel defaultSize={75} minSize={2}>
							<ResizablePanelGroup direction="vertical">
								<ResizablePanel defaultSize={60} minSize={2} className="flex flex-col">
									<WebSocketMessageLog
								...
								<ResizablePanel defaultSize={30} minSize={2} className="flex flex-col p-3 bg-muted/10">
						...
						<ResizableHandle withHandle className="bg-border/30 hover:bg-primary/20 transition-colors" />
						<ResizablePanel defaultSize={25} minSize={2} maxSize={30} className="bg-muted/5">
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-[11px] text-muted-foreground">Add parameters to the WebSocket URL</p>
							</div>
						</div>
						<ParameterComponent />
					</div>
