...
import { Loader2, Clock, HardDrive, FileCode, FileText, Cookie, Send, AlertCircle, Check } from 'lucide-react';
...
const ApiResponse = () => {
			...
			<div className="h-full overflow-auto p-2">
				<div className="flex flex-col gap-0.5">
					{Object.entries(response.headers).map(([key, value]) => (
						...
						>
							<span className="font-mono text-xs text-primary font-medium truncate">{key}</span>
							<span className="font-mono text-xs text-foreground/80 break-all">{value}</span>
						</div>
					))}
				</div>
			</div>
	...
	const renderCookies = () => {
		...
		const setCookieData = response?.setCookie || response?.headers?.['set-cookie'];

		if (!setCookieData || (Array.isArray(setCookieData) && setCookieData.length === 0)) {
			return (
				<div className="flex-1 flex flex-col items-center justify-center gap-3 py-12">
					<div className="size-12 rounded-xl bg-muted/50 border border-dashed border-border/50 flex items-center justify-center">
		...
		const cookieStrings = Array.isArray(setCookieData) ? setCookieData : [setCookieData];
		const requestDomain = response?.actualRequest?.url ? new URL(response.actualRequest.url).hostname : '';
			...
			<div className="h-full overflow-auto p-2">
				<div className="flex flex-col gap-2">
					{cookieStrings.map((cookieStr, index) => {
						...
						const isThirdParty = domain && requestDomain && !requestDomain.endsWith(domain.replace(/^\./, ''));
						...
						return (
							...
							>
								...
								<div className="flex items-start justify-between gap-2">
									<div className="flex flex-col gap-0.5 min-w-0">
										<div className="flex items-center gap-2">
											<span className="font-mono text-sm font-semibold text-primary truncate">{name}</span>
											{isThirdParty && (
								...
								<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 py-2 border-y border-border/30">
									<div className="flex flex-col gap-0.5">
										<span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Domain</span>
										<span className="font-mono text-xs truncate" title={domain as string || requestDomain}>{domain || requestDomain}</span>
									</div>
									<div className="flex flex-col gap-0.5">
										<span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Path</span>
										<span className="font-mono text-xs truncate">{attributes['path'] as string || '/'}</span>
									</div>
									<div className="flex flex-col gap-0.5 col-span-2">
										<span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">Expires</span>
										<span className="font-mono text-xs truncate">{attributes['expires'] as string || 'Session'}</span>
									</div>
								</div>
		...
		if (!response?.actualRequest) {
			return (
				<div className="flex-1 flex flex-col items-center justify-center gap-3 py-12">
					...
					<p className="text-sm text-muted-foreground">Send a request to see details</p>
				</div>
			);
		}
		...
		return (
			<div className="h-full overflow-auto p-3 space-y-3">
				...
				<div className="rounded-lg border border-border/50 bg-muted/20 p-3">
					<div className="text-[10px] uppercase tracking-wider text-muted-foreground/60 font-bold mb-2">Request URL</div>
					<div className="flex items-start gap-2">
						...
						<span className="font-mono text-xs text-foreground break-all">{url}</span>
					</div>
				</div>

				{/* Headers Section */}
				<div className="rounded-lg border border-border/50 bg-muted/20 p-3">
					<div className="text-[10px] uppercase tracking-wider text-muted-foreground/60 font-bold mb-2">Headers</div>
					<div className="flex flex-col gap-0.5">
						{Object.entries(headers || {}).map(([key, value]) => (
							<div key={key} className="grid grid-cols-[minmax(120px,auto)_1fr] gap-2 font-mono text-[11px]">
								<span className="text-primary font-medium">{key}:</span>
								<span className="text-foreground/80 break-all">{value}</span>
							</div>
						))}
						{Object.keys(headers || {}).length === 0 && (
							<span className="text-muted-foreground text-[11px] italic">No headers</span>
						)}
					</div>
				</div>
				...
				{cookies && cookies.length > 0 && (
					<div className="rounded-lg border border-border/50 bg-muted/20 p-3">
						<div className="text-[10px] uppercase tracking-wider text-muted-foreground/60 font-bold mb-2">Cookies</div>
						<div className="flex flex-col gap-1">
							{cookies.map((cookie: any, i: number) => (
								<div key={i} className="flex items-center gap-2 font-mono text-[11px]">
									<span className="text-primary font-medium">{cookie.key}</span>
									<span className="text-muted-foreground">=</span>
									<span className="text-foreground/80 truncate max-w-[200px]">{cookie.value}</span>
									<div className="flex gap-1 ml-auto">
										{cookie.secure && (
											<span className="px-1.5 py-0.5 rounded text-[9px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">Secure</span>
										)}
										{cookie.httpOnly && (
											<span className="px-1.5 py-0.5 rounded text-[9px] bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">HttpOnly</span>
										)}
									</div>
								</div>
							))}
				...
				{body && (
					<div className="rounded-lg border border-border/50 bg-muted/20 p-3">
						<div className="text-[10px] uppercase tracking-wider text-muted-foreground/60 font-bold mb-2">Body</div>
	...
	const renderLoading = () => (
		<div className="flex flex-col items-center justify-center h-full gap-3">
			...
			<span className="text-sm text-muted-foreground">Fetching response...</span>
		</div>
	);
		...
		return (
			<section className="flex h-full w-full flex-1 flex-col p-3">
				<div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 flex items-start gap-3">
					...
					<div>
						<p className="font-semibold text-rose-600 dark:text-rose-400 text-sm">Request Failed</p>
						<p className="mt-1 text-xs text-rose-600/80 dark:text-rose-400/80">{response.error}</p>
					</div>
				</div>
			</section>
		);
	...
	// Use raw setCookie array if available, otherwise fall back to header
	const setCookieData = response?.setCookie || response?.headers?.['set-cookie'];
	const cookiesCount = setCookieData ? (Array.isArray(setCookieData) ? setCookieData.length : 1) : 0;
	...
	return (
		<section className="flex h-full w-full flex-1 flex-col overflow-hidden">
			...
			{response && !isLoading && response.status > 0 && (
				<div className="flex items-center gap-3 px-3 py-2 border-b border-border/50 shrink-0">
					...
					<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
						...
						<span className="text-foreground font-medium">{formatSize(response.size)}</span>
					</div>
				</div>
			)}
			...
			>
				...
				{/* Tab Contents */}
				<TabsContent value="body" className="mt-0 flex-1 min-h-0 flex flex-col overflow-hidden">
					{isLoading ? (
						renderLoading()
					) : (
				...
				<TabsContent value="actual-request" className="mt-0 flex-1 overflow-auto">
					{renderActualRequest()}
				</TabsContent>
			</Tabs>
		</section>
