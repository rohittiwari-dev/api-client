...
import { Rocket, Search, ChevronUp, ChevronDown, X, Download, Sparkles, FileCode, Eye, TreeDeciduous } from 'lucide-react';
...
const ResponseBodyTabContent: React.FC<ResponseBodyTabContentProps> = ({
	...
	const scrollToMatch = useCallback((index: number, matches: number[], query: string) => {
		...
		const lineElements = contentRef.current.querySelectorAll('.line, [class*="line"]');
		...
		return (
			<div className="flex-1 flex flex-col items-center justify-center gap-4 py-16">
				...
				<div className="text-center space-y-1">
					...
					<p className="text-xs text-muted-foreground max-w-[200px]">Hit Send to get a response</p>
				</div>
			</div>
		);
	}
	...
	return (
		<Tabs defaultValue="pretty" className="flex flex-col h-full min-h-0 overflow-hidden">
			{/* Modern Tab Header */}
			<div className="flex w-full items-center gap-2 sticky top-0 z-10 bg-background py-1.5 select-none border-b border-border/30">
				...
				>
					...
					<SelectContent className="rounded-lg shadow-lg">
						<SelectItem value="json" className="text-xs cursor-pointer">JSON</SelectItem>
						<SelectItem value="xml" className="text-xs cursor-pointer">XML</SelectItem>
						<SelectItem value="html" className="text-xs cursor-pointer">HTML</SelectItem>
					...
					{searchQuery && (
						<div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
							<span className="text-[9px] text-muted-foreground/70 mr-1 font-medium">
								{searchResults.length > 0 ? `${currentMatchIndex + 1}/${searchResults.length}` : '0'}
							</span>
							<button
			...
			<div ref={contentRef} className="flex-1 min-h-0 overflow-hidden flex flex-col pt-1">
				<TabsContent value="pretty" className="flex-1 min-h-0 h-full mt-0 data-[state=active]:flex flex-col">
					{renderPrettyContent()}
				</TabsContent>
				<TabsContent value="raw" className="flex-1 min-h-0 h-full mt-0 data-[state=active]:flex flex-col">
					{renderRawContent()}
				</TabsContent>
				<TabsContent value="preview" className="flex-1 min-h-0 h-full mt-0 data-[state=active]:flex flex-col">
					{renderPreviewContent()}
				</TabsContent>
				<TabsContent value="visualize" className="flex-1 min-h-0 h-full mt-0 data-[state=active]:flex flex-col">
