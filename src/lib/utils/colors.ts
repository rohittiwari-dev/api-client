...
export function oklchToHex(input: string, fallback = "#ffffff"): string {
  ...
  try {
    ...
    const X =
      l * 1.227013851103521 - m * 0.5577999806518222 + s * 0.2812561489664678;
    const Y =
      l * -0.04058017842328059 +
      m * 1.1122568696168302 -
      s * 0.07167667866560119;
    const Z =
      l * -0.0763812845057069 - m * 0.4214819784180127 + s * 1.586163220440795;
    ...
    const bLinear =
      X * 0.05563007969699361 - Y * 0.2039769588889765 + Z * 1.0569715142428786;
...
export const requestTextColorMap: Record<HttpMethod, string> = {
  GET: "text-green-500",
  POST: "text-blue-500",
  PUT: "text-yellow-500",
  DELETE: "text-red-500",
...
export const requestBgColorMap: Record<HttpMethod, string> = {
  ...
  PATCH:
    "bg-orange-500 hover:bg-orange-600 hover:dark:bg-orange-800 dark:bg-orange-900",
};
